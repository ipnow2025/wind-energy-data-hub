...
export default function AdminEditPostPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (post) {
          ...
          setAuthor(post.author === "admin" ? "KIER(관리자)" : post.author || "알 수 없음")
          setCreatedAt(post.created_at || post.createdAt) // Use created_at instead of createdAt to match API response
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        router.push(`/admin/knowledge/post/${postId}`)
      } else {
            ...
            <form onSubmit={handleSubmit} className="space-y-6 p-6 border border-border rounded-lg bg-white">
              <div className="flex items-center justify-between pb-4 border-b">
                ...
                <div className="flex gap-2">
                  <Button
                    type="button"
                    onClick={() => router.push(`/admin/knowledge/post/${postId}`)}
