...
export default function AdminPostsPage() {
  ...
  const filteredPosts = posts.filter((post) => {
    ...
    if (appliedSearchType === "title") {
      ...
      const displayAuthor = post.author === "admin" ? "KIER(관리자)" : post.author
      return displayAuthor.toLowerCase().includes(query)
    }
    return true
  })
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold">지식센터 관리</h1>
        </div>
                  ...
                  {isLoading ? (
                    ...
                    currentPosts.map((post, index) => (
                      <tr key={post.id} className="border-b hover:bg-gray-50 cursor-pointer">
                        ...
                        <td className="p-4 text-left" onClick={() => router.push(`/admin/knowledge/post/${post.id}`)}>
