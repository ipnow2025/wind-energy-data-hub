...
export default function CollaborationPostDetailPage() {
  ...
  const fetchPost = async () => {
    try {
      console.log("[v0] CollaborationPostDetailPage - fetching post:", postId)
      ...
      console.log("[v0] CollaborationPostDetailPage - calling API with headers")
      const res = await fetch(`/api/collaboration/posts/${postId}`, { headers })
      console.log("[v0] CollaborationPostDetailPage - API response status:", res.status)

      if (res.ok) {
        const data = await res.json()
        console.log("[v0] CollaborationPostDetailPage - received data:", data)
        setPost(data.post)
      } else if (res.status === 401) {
        console.log("[v0] CollaborationPostDetailPage - unauthorized, redirecting to login")
        router.push("/login")
      } else if (res.status === 403) {
        console.log("[v0] CollaborationPostDetailPage - forbidden")
        alert("이 게시글에 접근할 권한이 없습니다.")
        router.push("/collaboration")
      } else {
        console.log("[v0] CollaborationPostDetailPage - post not found")
        alert("게시글을 찾을 수 없습니다.")
        router.push("/collaboration")
      }
    } catch (error) {
      console.error("[v0] CollaborationPostDetailPage - fetch error:", error)
  ...
  useEffect(() => {
    if (postId === "new") {
      console.log("[v0] Skipping fetch for 'new' route")
      setIsLoading(false)
      return
    }
    fetchPost()
  ...
  const canEditDelete =
    ...
    (post.author && currentUserId && post.author.toLowerCase() === currentUserId.toLowerCase())
      ...
      <section className="py-20 px-4">
        <div className="container mx-auto">
          <div className="max-w-5xl mx-auto">
            <div className="space-y-6 p-6 border border-border bg-white rounded-lg">
              <div className="flex items-center justify-between pb-4 border-b">
                ...
                <div className="flex gap-2">
                  ...
                  {canEditDelete && (
                    <>
                      <Button
                        onClick={() => router.push(`/collaboration/post/${postId}/edit`)}
                        disabled={isDeleting}
              ...
              <div className="grid grid-cols-2 gap-4 pb-4 border-b">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-base">
                      {post.board_id ? (post.board_id === "all" ? "전체 게시판" : post.board_id) : "알 수 없음"}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="min-h-[300px] p-4 rounded border whitespace-pre-wrap bg-background rounded-lg">
                  {post.content}
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {post.files.map((file, index) => (
                      <div key={index} className="p-3 rounded border bg-background space-y-2">
